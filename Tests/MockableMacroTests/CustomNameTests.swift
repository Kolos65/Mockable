//
//  CustomNameTests.swift
//
//
//  Created by Abraham Duran on 8/31/24.
//

import MacroTesting
@testable import Mockable

final class CustomNameTests: MockableMacroTestCase {
    func test_custom_name() {
        assertMacro {
          """
          @Mockable(name: "MockTestService")
          protocol Test {
              var foo: Int { get }
          ...
          """
            ...
            #if MOCKING
            final class MockTestService: Test, MockableService {
                private let mocker = Mocker<MockTestService>()
                ...
                struct ReturnBuilder: EffectBuilder {
                    private let mocker: Mocker<MockTestService>
                    init(mocker: Mocker<MockTestService>) {
                        self.mocker = mocker
                    }
                    var foo: FunctionReturnBuilder<MockTestService, ReturnBuilder, Int, () -> Int> {
                        .init(mocker, kind: .m1_foo)
                    }
                    func bar(number: Parameter<Int>) -> FunctionReturnBuilder<MockTestService, ReturnBuilder, Int, (Int) -> Int> {
                ...
                struct ActionBuilder: EffectBuilder {
                    ...
                    var foo: FunctionActionBuilder<MockTestService, ActionBuilder> {
                        .init(mocker, kind: .m1_foo)
                    }
                    func bar(number: Parameter<Int>) -> FunctionActionBuilder<MockTestService, ActionBuilder> {
                        .init(mocker, kind: .m2_bar(number: number))
                    }
                }
                struct VerifyBuilder: AssertionBuilder {
                    ...
                    init(mocker: Mocker<MockTestService>, assertion: @escaping MockableAssertion) {
                        self.mocker = mocker
                        self.assertion = assertion
                    }
                    var foo: FunctionVerifyBuilder<MockTestService, VerifyBuilder> {
                        .init(mocker, kind: .m1_foo, assertion: assertion)
                    }
                    func bar(number: Parameter<Int>) -> FunctionVerifyBuilder<MockTestService, VerifyBuilder> {
