...
* **`getCalled(count:)`**: Available for mutable properties only, asserts property access count.
* **`setCalled(count:)`**: Available for mutable properties only, asserts property assignment count.
